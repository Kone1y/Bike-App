...
- [x] 数据库迁移（sys_user 添加 openid 列）
- [x] 管理后台 Vite 降级 v5（兼容 Node 20）
- [x] 小程序 H5 模式支持（浏览器直接访问）
- [x] TabBar 图标生成
- [x] 前后端接口联调（登录、附近车辆、景点、租车、结束行程、订单、故障上报）
- [x] 租赁业务完整流程测试
- [x] Bug 修复（admin repair 页面 handleRepair 命名冲突、admin @ 路径别名缺失）
